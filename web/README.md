...
A self-contained, dependency-free **3D knowledge-graph explorer** for the knitweb (Three.js + d3-force-3d libraries are inlined; the default Plugins/Begrippen views need no CDN or internet). Three views via the top-left control:
...
- **Live chem-web** — fetches same-folder `explorer-graph.json` first (a tiny sample is included for clean static checkouts), then `/molgang/explorer-graph.json`, then `https://5mart.ml/molgang/explorer-graph.json`, and renders the molgang chemistry knowledge web in 3D.
